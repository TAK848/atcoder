...
int main()
{
    ...
    rep(i, H) rep(j, W)
    {
        ...
        x--;
        a.at(i).at(j) = x;
        chmax(nums.at(x).at(0), i);
        chmin(nums.at(x).at(1), i);
        chmax(nums.at(x).at(2), j);
        chmin(nums.at(x).at(2), j);
        // nums.at(i).at(1) = min()0, i);
        // nums.at(x)++;
    }
    rep(i, H) rep(j, W)
    {
    ...
    vvmi answer(H - h + 1, vmi(W - w + 1, 0));
    // rep(i, H - h + 1) rep(jj, W - w + 1)
    // {
    //     mi j = jj;
    //     if (i % 2 == 1) {
    //         j = W - w - jj;
