...
int main()
{
    ...
    startok.push_front(true);
    vb endok(tsize, false);
    rep(i, tsize)
    {
        char sc = s.at(ssize - 1 - i);
        char tc = t.at(tsize - 1 - i);
        if (sc == tc || sc == '?' || tc == '?')
            endok.at(i) = true;
        else {
            break;
    ...
    rep(i, tsize + 1)
    {
        cout << YesNo(startok.at(i) && endok.at(i)) << '\n';
    }
}
